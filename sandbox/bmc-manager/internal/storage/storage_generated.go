...
// LoadAllBMCCredentials loads all BMCCredential resources from Ent storage
func LoadAllBMCCredentials(ctx context.Context) ([]*v1.BMCCredential, error) {
	if entClient == nil {
	...
	entResources, err := entClient.Resource.Query().
		Where(entresource.KindEQ("BMCCredential")).
		WithLabels().
		WithAnnotations().
		All(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to load BMCCredential resources: %w", err)
	}

	// Convert to Fabrica resources
	var resources []*v1.BMCCredential
	for _, entResource := range entResources {
		...
		resources = append(resources, fabricaResource.(*v1.BMCCredential))
	}

	return resources, nil
}

// LoadBMCCredential loads a single BMCCredential resource by UID from Ent storage
func LoadBMCCredential(ctx context.Context, uid string) (*v1.BMCCredential, error) {
	if entClient == nil {
		return nil, fmt.Errorf("ent client not initialized")
	}
	...
	entResource, err := entClient.Resource.Query().
		Where(
			...
			entresource.KindEQ("BMCCredential"),
		).
		WithLabels().
		WithAnnotations().
		Only(ctx)
	...
	return fabricaResource.(*v1.BMCCredential), nil
}

// SaveBMCCredential saves a BMCCredential resource to Ent storage
func SaveBMCCredential(ctx context.Context, resource *v1.BMCCredential) error {
	if entClient == nil {
	...
	if err != nil {
		return fmt.Errorf("failed to convert BMCCredential to ent: %w", err)
	}
	...
	if err != nil && !ent.IsNotFound(err) {
		return fmt.Errorf("failed to check BMCCredential existence: %w", err)
	}
...
// DeleteBMCCredential deletes a BMCCredential resource from Ent storage
func DeleteBMCCredential(ctx context.Context, uid string) error {
	if entClient == nil {
		return fmt.Errorf("ent client not initialized")
	}
	...
	if err != nil {
		return fmt.Errorf("failed to delete BMCCredential %s: %w", uid, err)
	}
...
func (b *entStorageBackend) LoadAll(ctx context.Context, resourceType string) ([]json.RawMessage, error) {
	...
	switch resourceType {
	case "BMCCredential":
		resources, err := LoadAllBMCCredentials(ctx)
		if err != nil {
			return nil, err
		}
...
func (b *entStorageBackend) Load(ctx context.Context, resourceType, uid string) (json.RawMessage, error) {
	...
	switch resourceType {
	case "BMCCredential":
		resource, err := LoadBMCCredential(ctx, uid)
		if err != nil {
			return nil, err
		}
...
func (b *entStorageBackend) Save(ctx context.Context, resourceType, uid string, data json.RawMessage) error {
	...
	case "BMCCredential":
		...
		if err := json.Unmarshal(data, &res); err != nil {
			return fmt.Errorf("failed to unmarshal BMCCredential: %w", err)
		}
		res.Metadata.UID = uid
		return SaveBMCCredential(ctx, &res)
	default:
...
func (b *entStorageBackend) Delete(ctx context.Context, resourceType, uid string) error {
	...
	switch resourceType {
	case "BMCCredential":
		return DeleteBMCCredential(ctx, uid)
	default:
		return fmt.Errorf("storage: unsupported resource type %s", resourceType)
	}
}
...
func (c *StorageClient) Get(ctx context.Context, kind, uid string) (interface{}, error) {
	...
	switch kind {
	case "BMCCredential":
		var resource v1.BMCCredential
		if err := json.Unmarshal(raw, &resource); err != nil {
			return nil, fmt.Errorf("failed to unmarshal BMCCredential: %w", err)
		}
		return &resource, nil
	default:
...
func (c *StorageClient) Update(ctx context.Context, resource interface{}) error {
	...
	case *v1.BMCCredential:
		return c.backend.Save(ctx, "BMCCredential", res.Metadata.UID, data)
	default:
