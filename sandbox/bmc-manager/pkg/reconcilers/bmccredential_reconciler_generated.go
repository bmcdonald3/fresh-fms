...
// This file provides the generated boilerplate for BMCCredential reconciler.
//
...
//
// To customize reconciliation logic, edit bmccredential_reconciler.go
package reconcilers
...
//
// This reconciler:
//   - Observes BMCCredential resources and updates their Status
...
//
// The implementation of reconcileBMCCredential() is in bmccredential_reconciler.go
type BMCCredentialReconciler struct {
...
//
// Returns:
//   - *BMCCredentialReconciler: Initialized reconciler
func NewDefaultBMCCredentialReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *BMCCredentialReconciler {
	return &BMCCredentialReconciler{
...
func (r *BMCCredentialReconciler) GetResourceKind() string {
	return "BMCCredential"
}

// Reconcile brings BMCCredential to desired state.
//
// This method is called:
//   - When a BMCCredential resource is created/updated/deleted
...
//   - resource: The BMCCredential resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *BMCCredentialReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.BMCCredential // This is the typed struct
	...
	r.Logger.Debugf("Reconciling BMCCredential %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileBMCCredential(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for BMCCredential %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for BMCCredential %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.bmccredentials.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for BMCCredential %s: %v", res.GetUID(), err)
