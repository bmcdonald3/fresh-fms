...
Examples:
  ...
  bmc_manager export --format yaml --output ./backup

  # Export specific resource types
  bmc_manager export --kinds BMCCredential --output ./bmccredential-backup
`,
...
func runExport(ctx context.Context, format, output string, kinds []string, perType bool) error {
	...
	if len(kinds) > 0 {
		...
		resourceKinds = []string{"BMCCredential"}
	}
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "BMCCredential":
		items, e := storage.Querybmccredentials(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query bmccredentials: %w", e)
		}
